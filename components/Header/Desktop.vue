...
<template>
  <header class="mx-auto hidden h-20 w-full max-w-screen-xl items-center justify-between gap-4 rounded-full border border-gray-100/15 bg-background/15 bg-clip-padding p-4 backdrop-blur-lg backdrop-filter lg:flex">
    ...
    <nav class="flex flex-1 items-center justify-center">
      <ul class="flex items-center gap-3">
        <li>
          <Button class="text-lg text-foreground" variant="link" @click="() => emit('navigate-to-home')">
            Home
          </Button>
        </li>
        <li>
          <Button class="text-lg text-foreground" variant="link" @click="() => emit('navigate-to-speakers')">
            Palestrantes
          </Button>
        </li>
        <li>
